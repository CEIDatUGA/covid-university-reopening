...
The transmission rate $\beta$ within the university population is estimated from the expression for $\mathcal{R}_{0}$, by setting $\xi$ to 0 and solving for $\beta$,
...
Our notional case assumes the basic reproduction number in the absence of a testing program is $\mathcal{R}_{0}= 3.0$ @Park2020-vl, although we relax this assumption in sensitivity analysis. For the notional case, the estimated transmission rate is $\beta =$ `r beta`.
...
We do not currently have a reliable method to estimate the effectiveness of these interventions empirically, and instead estimate effectiveness using a conceptual model that considers transmission across three basic interaction types by three primary transmission routes.
...
For $m=$ `r scales::comma(N_stud)`  students and $n=$ `r scales::comma(N_staff)` staff, there are $m^2-m \approx 1.6 \text{ billion}$ possible student-student interactions, $m \times n \approx 400 \text{ million}$ student-staff interactions, and $n^2-n \approx 100 \text{ million}$ staff-staff interactions for a total of `r scales::comma(inter_tot)` possible interactions.
...
p <- ggplot() +
  ...
  ylab("Probability") +
  ggtitle("Probability that a class contains an infected person") +
  labs(color = "Estimate")
...
text(x=0.25, y=8900, 'Estimated intervention effectiveness', col='#19bdc2', pos=4)
...
To isolate the effect of uncertainty in epidemiological parameters on the critical testing rate, we varied the transmission parameter $\beta$ so that $\mathcal{R}_0$ ranged from $\mathcal{R}_0=2$ (optimistic) to $\mathcal{R}_0=4$ (pessimistic) and varied the remaining epidemiological parameters by +/- $`r scales::percent(variation)`$%.
This was accomplished by Latin Hypercube sampling from uniform distributions representing the range of their possible values as follows: 
...
We performed Latin Hypercube sampling with $`r num_samples`$ samples to efficiently explore the effects of varying the parameter values on the critical testing rate (Fig. \ref{fig:xi-sensitivity}).
